...
class ConfMatrix(object):
    ...
    def update(self, pred, target):
        ...
        with torch.no_grad():
            k = (target >= 0) & (target < n)
            #print(torch.min(target[k]), torch.max(target[k]))
            #print(torch.min(pred[k]), torch.max(pred[k]))
    ...
    def compute_metric(self, only_pri=False):
        ...
        if self.include_mtl:
            ...
            if self.dataset == 'nyuv2':
                ...
                stl = {'semantic': 0.0, 'depth': 0.0}

            delta_mtl = 0
            for task_id in self.train_tasks:
                if task_id in ['seg', 'part_seg', 'semantic'] or 'class' in task_id:  # higher better
                    delta_mtl += (self.metric[task_id][e, 1] - stl[task_id]) / stl[task_id]
                elif task_id in ['depth', 'normal', 'disp', 'noramls']:
                    delta_mtl -= (self.metric[task_id][e, 1] - stl[task_id]) / stl[task_id]
